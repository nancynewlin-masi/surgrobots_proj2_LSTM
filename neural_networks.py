...
class LSTMEncoder(nn.Module):
    def __init__(self, input_size, hidden_size = 512, num_layers = 1, optical_flow_stream = False):
    ...
    def forward(self, x_input):
        ...
        '''
        #x_input = self.conv1(x_input) # uncomment if size issue 
        lstm_out, self.hidden = self.lstm(x_input.view(x_input.shape[0], x_input.shape[1], self.input_size))
        lstm_out = lstm_out.view(-1,512*lstm_out.size()[2]*lstm_out.size()[3])
...
class encoderDecoder(nn.Module):
    def __init__(self, input_size, embedding_dim: int) -> None:
        super().__init__()
        self.input_size = input_size
        self.hidden_size = embedding_dim
        self.conv_net_stream = LSTMEncoder(input_size = input_size, hidden_size = embedding_dim)
        self.decoder = LSTMDecoder(input_size = input_size, hidden_size = embedding_dim)
